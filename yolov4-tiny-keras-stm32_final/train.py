...
if __name__ == "__main__":
    ...
    config.gpu_options.per_process_gpu_memory_fraction = 0.6  # 指定显存分配比例
    ...
    #   创建yolo模型
    #------------------------------------------------------#
    image_input = Input(shape=(input_shape[1], input_shape[0], 3))
    h, w = input_shape
    ...
    #global a
    a = 8
    y_true = Input(shape=(h//a, w//a, num_anchors, num_classes+5)),
    print(model_body.output)
    print(y_true)
    outputofbody=model_body.output,
    print(outputofbody)
    loss_input = [*outputofbody, *y_true]
    ...
    #-------------------------------------------------------------------------------#{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}
    logging = TensorBoard(log_dir=log_dir)
    checkpoint = ModelCheckpoint(log_dir + 'ep.h5',
    ...
    if True:
        ...
        model.save('model_data/yolo07013.h5')
